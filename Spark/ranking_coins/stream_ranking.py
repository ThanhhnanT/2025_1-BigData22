...
from pyspark.sql.functions import from_json, col, max, desc, current_timestamp, to_json, struct
...
CHECKPOINT_PATH = "/tmp/spark/realtime_ranking_cp"  # consider using durable storage (HDFS/S3) in production
...
# Compute latest (by timestamp 't') close price per symbol, plus trade count and updated timestamp
ranking_df = processed_df \
    .groupBy("s") \
    .agg(
        F.max(F.struct(col("t"), col("close_price_num"))).alias("latest_struct"),
        F.count("s").alias("trade_count"),
        F.current_timestamp().alias("updated_at")
    ) \
    .select(
        col("s"),
        col("latest_struct").getField("close_price_num").alias("latest_price"),
        col("trade_count"),
        col("updated_at")
    ) \
...
# prepare output for Kafka sink: Kafka requires 'key' and 'value' columns (both strings/binary).
output_df = ranking_df.select(
    col("s").alias("key"),
    to_json(F.struct("s","latest_price","trade_count","updated_at")).alias("value")
)
