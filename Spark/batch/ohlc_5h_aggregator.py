...
def main():
    ...
    for idx in existing_indexes:
        ...
        if (
            ...
            index_exists = True
            print(f"  ℹ️  5h index already exists: {idx.get('name', 'unnamed')}")
            break
    ...
    if not index_exists:
        try:
            ...
            print("  ✅ 5h index created successfully")
        except OperationFailure as e:
            if e.code == 85:
                print("  ℹ️  5h index already exists with different name, skipping creation")
            else:
                print(f"  ⚠️  MongoDB 5h index creation error (code {e.code}): {e}")
        except Exception as e:
            print(f"  ⚠️  Unexpected error creating 5h index: {e}")
