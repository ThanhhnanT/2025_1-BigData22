...
Aggregates 5m kline data from MongoDB into 1h OHLC candles
"""
...
def main():
    ...
    print(f"🚀 Starting OHLC 1h Aggregator - Spark Job")
    ...
    # Load 5m data from MongoDB for all symbols
    query = {
        "symbol": {"$in": SYMBOLS},
        "interval": "5m",
    ...
    if not klines:
        print("❌ No 5m kline data found")
        spark.stop()
        mongo_client.close()
        return
    
    print(f"✅ Loaded {len(klines)} 5m klines from MongoDB")
    ...
    # Calculate 1h window start time for grouping (floor to 1h intervals)
    df = df.withColumn(
        "window_start",
        (col("openTime") / 3600000).cast("long") * 3600000
    )
    
    # Group by symbol and 1h window, aggregate OHLC
    ...
    print(f"📊 Aggregated into {len(results)} 1h candles")
    ...
    for row in results:
        ...
        if existing:
            print(f"  ℹ️  1h OHLC already exists for {row['symbol']} (openTime: {row['openTime']})")
            total_skipped += 1
            continue
        ...
        try:
            ...
            print(f"  ✅ {row['symbol']}: 1h OHLC saved (O:{row['open']:.4f}, H:{row['high']:.4f}, L:{row['low']:.4f}, C:{row['close']:.4f})")
            total_inserted += 1
        except Exception as e:
